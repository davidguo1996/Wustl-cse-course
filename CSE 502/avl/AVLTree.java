...
public class AVLTree<T extends Comparable<T>> {
	...
	private TreeNode<T> rightRotate(TreeNode<T> root) {
	    TreeNode<T> leftRoot = root.left;
	    root.setLeft(leftRoot.right);
	    // update children changed root's height
	    updateHeight(root);
	    leftRoot.setRight(root);
	    updateHeight(leftRoot);
	    return leftRoot;
	}
	...
	private TreeNode<T> leftRotate(TreeNode<T> root) {
	    TreeNode<T> rightRoot = root.right;
	    root.setRight(rightRoot.left);
	    // update children changed root's height
	    updateHeight(root);
	    rightRoot.setLeft(root);
	    updateHeight(rightRoot);
	    return rightRoot;
	}
