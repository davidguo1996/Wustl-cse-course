...
public class Sort {
	...
	public static int findIndexOfLexicographicallyEarliestValue(String[] array, int fromIndex) {
		...
		for (int i = fromIndex; i < array.length; ++i) {
			if (Strings.isLessThan(array[i], array[x])){
				x = i;
			}
		}
		return x;
	...
	public static void selectionSortInPlace(String[] array) {
		for (int i = 0; i < array.length; ++i) {
			int bIndex = findIndexOfLexicographicallyEarliestValue(array, i);
			if(Strings.isGreaterThan(array[i], array[bIndex])) {
				swapValuesAtIndicesInPlace(array, i, bIndex);
			}
	...
	public static boolean isSorted(String[] array) {
		...
		for (int i = 0; i < array.length - 1; ++i) {
			if (Strings.isGreaterThan(array[i], array[i+1])) {
				sorted = false;
			}
		}
		return sorted;
